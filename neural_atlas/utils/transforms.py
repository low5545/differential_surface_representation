...
class PclNormalization(Transform):
    ...
    def _before_apply(self, pcl):
        ...
        if self.mode == "ball":
            ...
            # MODIFIED: `torch.linalg.vector_norm()` is not supported in
            #           PyTorch 1.6
            unit_scale = torch.norm(                            # (..., 1, 1)
                centered_pcl, dim=-1, keepdim=True, p=None
...
"""
class PclMargins(Transform):
    def __init__(self, bounding_mode, margins):
        super().__init__()
        assert bounding_mode in [ "ball", "cube" ]
        assert margins >= 0.
