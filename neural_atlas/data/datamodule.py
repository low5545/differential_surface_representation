...
class DataModule(pl.LightningDataModule):
    ...
    def _init_transforms(self):
        ...
        NOTE:
            ...
            3. Target surface normal & surface area are always available during
               evaluation
        """
        ...
        # .class_id.common`, `self.transforms.train/eval.sample_id.common`,
        # `self.transforms.eval.nml.target` & `self.transforms.eval.area
        # .target` are always available
        ...
        if "area" not in self.target:
            self.transforms.train.pop("area")
        ...
        """
        A possible way of implementing such data-dependent transforms (here,
        the point cloud transforms are dependent on the camera intrinsics and 
        extrinsics of each image) is to apply a `StorageTerminal` transform on
        the dependent-data (e.g. `StorageTerminal.save()` on the camera 
        intrinsics and extrinsics). In the `Dataset`, data transformed with  
        `Terminal` transforms will be discarded from being output (e.g. by 
        checking with `isinstance()`). Then, data-dependent transforms can be 
        applied as `LoadingOperation` transforms (eg. 
        `LoadingRotation.apply()` with `StorageTerminal.load()`).

        Alternatively, extend `Transform` with `apply_loaded()`.
        """
    ...
    def _build_dataset(self, stage):
        ...
        NOTE:
            ...
            2. Surface normal & area are always available during evaluation
        """
        modalities = set(
