...
class ShapeNetSurfaceArea(dataset_mixins.SingleModalityDataset):
    IMAGE_FOLDER_NAME = "img_choy2016"
    SURFACE_AREA_FILENAME = "surface_area.npy"
    ...
    def _getitem_from_dir(cls, directory):
        if cls.IMAGE_FOLDER_NAME in directory:
            # remove trailing ".../img_choy2016/0xx"
            directory = os.path.dirname(os.path.dirname(directory))
        filepath = os.path.join(directory, cls.SURFACE_AREA_FILENAME)
        surface_area = np.load(filepath)

        # loaded point cloud `ndarray` has a data-type of `float64`
        return torch.from_numpy(surface_area).to(torch.get_default_dtype())
